...
async function call(body,method='PATCH') {
 let status,payload;
 await handler({method,headers:{authorization:'Bearer fixture'},body},{status(s){status=s;},setHeader(){},send(s){payload=JSON.parse(s);}});
...
globalThis.fetch=async (input,options={})=>{
 ...
 if(url.pathname==='/rest/v1/profiles')return ok(profiles);

...
import {markerStyle} from './markers.js';
...
export async function mountLocator(root,client,role=false,network=[]) {
 ...
 ${manage?'<div class="locator-actions"><button id="sl-new">Aggiungi struttura</button><label class="sl-upload">Importa Excel<input id="sl-file" type="file" accept=".xlsx"></label><button id="sl-refresh" class="secondary">Aggiorna</button><span>Excel .xlsx · massimo 500 strutture · 5 MB</span></div>':''}
 <p id="sl-message" role="status"></p><div class="locator-filters"><label>Cerca<input id="sl-search" type="search" placeholder="Nome, città o CAP"></label><label>Paese<select id="sl-country"><option value="">Tutti i Paesi</option></select></label><label>Tipologia<select id="sl-type"><option value="">Tutte le tipologie</option>${Object.entries(categories).map(([k,v])=>`<option value="${k}">${v}</option>`).join('')}</select></label>${manage?'<label>Stato<select id="sl-state"><option value="">Tutte le schede</option><option value="public">Attive e approvate</option><option value="draft">Da approvare / inattive</option><option value="unplaced">Da posizionare</option></select></label>':''}</div>
 <p id="sl-count"></p><div class="locator-layout ${manage?'':'locator-map-only'}"><div id="sl-list" class="locator-list" ${manage?'':'hidden'}></div><div id="sl-map" class="locator-map" aria-label="Mappa strutture ODR"></div></div>
 ...
 async function load() {
  ...
  try {
   let all=[];
   for(let from=0;;from+=1000){const {data,error}=await (manage&&!admin ? client.rpc('my_store_locations').range(from,from+999) : client.from('store_locations').select('*').order('name').order('id').range(from,from+999));if(error)throw error;all.push(...data);if(data.length<1000)break;}
   if(disposed)return;
   stores=all;
 ...
 function card(r) {
  ...
  return `<article class="locator-card"><div class="locator-tags">${r.categories.map(c=>`<span>${esc(categories[c])}</span>`).join('')}</div><h3>${esc(r.name)}</h3><p>${esc([r.address,r.postcode,r.city,r.province,r.country].filter(Boolean).join(', '))}</p>${r.phone||r.mobile?`<p>${esc(r.phone||r.mobile)}</p>`:''}${r.email?`<a href="mailto:${esc(r.email)}">${esc(r.email)}</a>`:''}<div class="locator-actions"><a href="${directions}" target="_blank" rel="noopener">Indicazioni stradali ↗</a>${url?`<a href="${esc(url)}" target="_blank" rel="noopener">Sito web ↗</a>`:''}${manage?`<button data-edit="${r.id}">Modifica</button>`:''}</div>${manage?`<small>${r.active&&r.approved?'Attiva e approvata':'Inattiva / da approvare'}${!positioned(r)?' · Da posizionare sulla mappa':''}</small>`:''}</article>`;
 }
 function render() {
 ...
 q('#sl-form').addEventListener('submit',async e=>{
  e.preventDefault();if(!manage||busy)return;
  ...
  try{const internal=Object.fromEntries(['reference','contact','notes','owner_entity_id'].map(k=>[k,String(f.get(k)||'')]));const {error}=await client.rpc('save_store_locations',{entries:[{row,internal}]});if(error)throw error;q('#sl-dialog').close();await load();message(admin?'Scheda salvata.':'Scheda salvata e inviata all’amministratore per approvazione.');}catch(err){q('#sl-form-message').textContent=err.code==='23505'?'Esiste già una struttura con questo nome e indirizzo.':'Salvataggio non riuscito. Controlla i dati e riprova.';}finally{busy=false;button.disabled=false;}
 });
 ...
 q('#sl-confirm').addEventListener('click',async()=>{
  if(busy||!manage)return;busy=true;q('#sl-confirm').disabled=true;

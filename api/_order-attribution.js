...
export function createAttributionIndex(network, profiles, accounts, appCustomers) {
  ...
  const resolve = order => {
    ...
    if (manual?.mode === 'manual') {
      ...
      const explicitEntity = metaValue(order, '_odr_agent_entity_id');
      const explicitProfile = metaValue(order, '_odr_agent_profile_id');
      entity = byId.get(explicitEntity)
        || byProfile.get(explicitProfile)
        || appById.get(metaValue(order, '_odr_customer_reference'));
      // An explicit owner outside the reader's scope must not fall back to a
      // matching customer and expose the order to a different agent.
      if (!entity && (explicitEntity || explicitProfile)) {
        return { agent: null, distributor: null, entityId: explicitEntity || '', source, conflict: true };
      }
      if (!entity) {
...
export async function loadAttributionIndex(wooAuthorization, profile) {
  // Use the same authenticated, role-scoped endpoint as the working network UI.
  // No privileged Vercel environment variable is required.
  if (!profile?.headers) throw new Error('Sessione associazioni non disponibile');
  const headers = profile.headers;
  const networkResponse = await fetch(`${process.env.SUPABASE_URL}/functions/v1/network-management`, {
    headers, cache: 'no-store', signal: AbortSignal.timeout(15000),
  });
  if (!networkResponse.ok) throw new Error('Rete commerciale non disponibile');
  const payload = await networkResponse.json();
  if (!Array.isArray(payload.entities)) throw new Error('Risposta rete commerciale non valida');
  ...
  const network = payload.entities;
  const visibleProfiles = [...new Map([...(payload.accounts || []), ...profiles, profile].map(item => [item.id, item])).values()];
  const index = createAttributionIndex(network, visibleProfiles, [], customers);
  const warnings = [];
